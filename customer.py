...
class Cust_Win:
    def __init__(self, root):
        ...
        entry_ref = ttk.Entry(labelframeleft, width=29, textvariable=self.var_ref, font=("arial", 13, "bold"),
                              state="readonly")
        ...
        combo_gender = ttk.Combobox(labelframeleft, width=27, textvariable=self.var_gender, font=("arial", 12, "bold"),
                                    state="readonly")
        ...
        combo_Nationality = ttk.Combobox(labelframeleft, width=27, textvariable=self.var_nationality,
        ...
        combo_id = ttk.Combobox(labelframeleft, width=27, textvariable=self.var_id_proof, font=("arial", 12, "bold"),
                                state="readonly")
        combo_id["value"] = ("Select", "AdharCard", "PAN", "DrivingLicence", "Passport")
        combo_id.current(0)
        ...
        scorll_x.config(command=self.Cust_Details_Table.xview)
        scorll_y.config(command=self.Cust_Details_Table.yview)
    ...
    def add_data(self):
        if self.var_mobile.get() == "" or self.var_mother.get() == "":
            ...
            try:
                ...
                messagebox.showwarning("Warning", f"Due to:{str(es)}", parent=self.root)
    ...
    def update(self):
        if self.var_mobile.get() == "":
            ...
            conn = mysql.connector.connect(host="localhost", username="root", password="Password@21",
                                           database="management")
            my_cursor = conn.cursor()
            my_cursor.execute("update customer set Name=%s, Mother=%s, Gender=%s, PostCode=%s, Mobile=%s, Email=%s, "
